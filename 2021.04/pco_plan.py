...
class pco_plan_update:
    ...
    def get_plan_app_cue_note_category_id(*self, service_type):
        logging.debug('get_plan_app_cue_note_category_id. Service type:  %s', service_type)
    ...
    def get_service_cateogry_app_cue_note_category_id(*self, service_type):
        logging.debug('get_app_ceu_note_category_id called. service type: %s', service_type)
        r = requests.get(f'https://api.planningcenteronline.com/services/v2/service_types/{service_type}/item_note_categories',
                         auth=(APP_ID, SECRET))
        r = json.loads(r.text)
    ...
    def check_if_plan_app_cue_exists(*self, service_type, plan):
        logging.debug('check_if_plan_app_cue_exists: service type: %s, plan: %s', service_type, plan)
...
# if __name__ == '__main__':
    # pco_plan_update.check_if_plan_app_cue_exists(service_type=824571, plan=51974942)
