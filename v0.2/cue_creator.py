...
def cue_creator_window(plan_items, item_index, service_type, plan_id):
    def main_ui(current_items):
        ...
        if current_items is None:
            ...
            existing_items_verbose = cue_coder.cue_verbose_decoder(cuelist= current_items)
    ...
    def update_cues_display(cue_dict):
        ...
        text_to_append = cue_coder.cue_verbose_decoder(cuelist=cue_dict)
        text_to_append = text_to_append[len(text_to_append)-1]
        logging.debug('Text to append to current_cues_label is %s, type %s', text_to_append, type(text_to_append))
    ...
    def finished(cue_dict):
        ...
        logging.debug('Successfully added cues to planning center. %s', cues)
        cues.clear()
        root.destroy()
        cue_creator.destroy()
    ...
    def startup():
        ...
        if not plan_items[item_index]['app_cues'] is None:
            ...
            if yes_no == False:
                ...
                main_ui(current_items=plan_items[item_index]['app_cues'])
        else:
