...
def service_selected(service_type_id, service_index):
    ...
    for iteration, item_note in enumerate(item_note_ids):
        ...
        item_ids = pco_plan.get_item_note_id(service_type=service_type_id, plan=service_id, item_id=item_note)
    ...
    for iteration, item in enumerate(items_master):
        cue_list = cue_coder.cue_verbose_decoder(cuelist=items_master[iteration]['app_cues'])
        cue_to_append = str()
...
def create_ui(plan_items):
    ...
    set_reminder(reminder_time=5, reminder='this is a test reminder')
    ...
    def options_button_pressed(item_index):
        ...
        cue_creator_window(plan_items=plan_items,
    ...
    btn_previous_without_actions = Button(service_controls_frame,
                          text='PREVIOUS (no actions)',
                          bg=accent_color_1,
                          fg=accent_text_color,
                          font=(accent_text_font, producer_note_text_size),
                          command=lambda: previous(cue_items=False)).grid(row=0, column=0)

    logging.debug('adding previous button')
    btn_previous = Button(service_controls_frame,
                          text='PREVIOUS',
                          bg=accent_color_1,
                          fg=accent_text_color,
                          font=(accent_text_font, accent_text_size),
                          command=lambda: previous(cue_items=True)).grid(row=0, column=1)

    logging.debug('Adding next button')
    btn_next = Button(service_controls_frame,
                      text='NEXT',
                      bg=accent_color_1,
                      fg=accent_text_color,
                      font=(accent_text_font, accent_text_size),
                      command=lambda: next(cue_items=True)).grid(row=0, column=2)

    logging.debug('Adding next (no actions) button.')
    btn_next_without_actions = Button(service_controls_frame,
                          text='NEXT (no actions)',
                          bg=accent_color_1,
                          fg=accent_text_color,
                          font=(accent_text_font, producer_note_text_size),
                          command=lambda: next(cue_items=False)).grid(row=0, column=3)

    aux_controls_frame.grid(sticky='e', row=0, column=1)
    clock_frame.grid(sticky='w', row=0, column=0)
    service_plan_frame.grid(row=2, column=0)
    service_controls_frame.grid(row=3, column=0)
    refresh_live_ui()
