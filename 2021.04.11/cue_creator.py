...
class CueCreator:
        ...
        try:
            if 'App Cues' in input_item['notes']:
                ...
                if yes_no: # User chose to delete existing cues
                    logging.debug('create_cues: Overwriting existing cues')
    ...
    def verbose_decode_cues(self, cuelist):
        ...
        if not cuelist == None:
            for cue in cuelist:
                logging.debug('creating verbose output from %s', cue)
                # cg3/cg4
    ...
    def activate_cues(self, cues):
        logging.debug('activate_cues called, cues input: %s', cues)
        for cue in cues:
            # pvp
            if cue['device'] in ('CG3', 'CG4'):
                if cue['device'] == 'CG3':
                ...
                if cue['device'] == 'CG4':
                    ip = cg4_ip
                    port = cg4_port
                logging.debug('activate_cues: cueing PVP %s cue, %s', cue['device'], cue['cue_name'])
            ...
            # rosstalk
                ...
                if cue['type'] == 'CC':
                    ...
                    logging.debug('activate_cues: cueing rosstalk: %s', command)
            ...
            # kipro
                ...
                if cue['start']:
                    if not cue['kipro'] == 0:
                        logging.debug('activate_cues: starting single kipro %s', cue['kipro'])
                        kipro.start_absolute(ip=kipro_data[cue['kipro']]['ip'],
                                             name=kipro_data[cue['kipro']]['name'],
                                             include_date=True)
                    if cue['kipro'] == 0:
                        logging.debug('activate_cues: starting all kipros')
                ...
                if not cue['start']:
                    if not cue['kipro'] == 0:
                        logging.debug('activate_cues: stopping single kipro: %s', cue['kipro'])
                        kipro.transport_stop(ip=kipro_data[cue['kipro']]['ip'])
                    if cue['kipro'] == 0:
                        logging.debug('activate_cues: stopping all kipros')
                        for kipro_number in range(1, len(kipro_data)):
                            kipro.transport_stop(ip=kipro_data[kipro_number]['ip'])
            # Resi
            elif cue['device'] == 'Resi':
                logging.debug('activate_cues: resi: %s', cue['command'])
                rt(rosstalk_ip=resi_ip, rosstalk_port=resi_port, command=cue['command'])
            else:
                logging.debug('Received cue not in activate_cues list: %s', cue)
                pass
    ...
    def __update_cues_display(self):
        logging.debug('Updating Cues Display: Input %s', self.current_cues)
        self.cues_display_text = str()
    ...
    def __add_to_pco(self):
        logging.debug('cue_creator.__add_to_pco: attempting to add cues to pco. service_type_id: %s, service_id: %s, item_id: %s, items: %s',
                      self.service_type_id, self.plan_id, self.input_item_id, self.current_cues)
        cues_json = json.dumps(self.current_cues)
        self.pco_plan.create_and_update_item_app_cue(item_id=self.input_item_id, app_cue=cues_json)
