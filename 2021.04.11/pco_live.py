...
class PcoLive:
    ...
    def go_to_next_item(self):
        logging.debug('Going to next item plan---- service type: %s, plan: %s', self.service_type_id, self.plan_id)
        ...
        try:
            ...
            logging.info('go_to_next_item: success')

    def go_to_previous_item(self):
        logging.debug('Going to previous item plan---- service type: %s, plan: %s', self.service_type_id, self.plan_id)
        r = requests.post(f'https://api.planningcenteronline.com/services/v2/service_types/'
        ...
        try:
            ...
            logging.info('go_to_previous_item: success')

    # simply toggles control. Use in conjunction with is_controlled for more use
    def toggle_control(self):
        logging.debug('Toggling control...---- service type: %s, plan: %s', self.service_type_id, self.plan_id)
        r = requests.post(f'https://api.planningcenteronline.com/services/v2/service_types/'
    ...
    def get_current_live_item(self):
        ...
        try:
            ...
            logging.error('Failed to get current live item id for service type %s, plan %s. Is the plan live?', self.service_type_id, self.plan_id)
            return None
        except KeyError:
            logging.error('Failed to get current live item id for service type %s, plan %s. Is the plan live?', self.service_type_id, self.plan_id)
            return None
    ...
    def get_current_live_item_time(self):
        logging.debug('getting live item time of service_type %s, plan %s', self.service_type_id, self.plan_id)
        r = requests.get(f'https://api.planningcenteronline.com/services/v2/service_types/'
                         f'{self.service_type_id}/plans/{self.plan_id}/live/current_item_time',
                         auth=(APP_ID, SECRET))
        data = json.loads(r.text)
    ...
    def get_current_plan_live_time_id(self):
        logging.debug('get_current_plan_live_time_id: service_type_id: %s, plan_id: %s', self.service_type_id, self.plan_id)
        r = requests.get(f'https://api.planningcenteronline.com/services/v2/service_types/'
                         f'{self.service_type_id}/plans/{self.plan_id}/live/current_item_time/', auth=(APP_ID, SECRET))
        r = json.loads(r.text)
        try:
            logging.debug('Found plan live time id: %s', r['data']['relationships']['plan_time']['data']['id'])
            return r['data']['relationships']['plan_time']['data']['id']
        except KeyError:
            logging.info('get_current_plan_live_time_id: something went wrong, response: ', r)
            ...
            if current_live_id < service['id']:
                if not has_advanced:
                    ...
                    if self.get_current_live_item() == first_item_id:
                        logging.debug('go_to_next_service: success')
            if iteration == len(live_service_info) and has_advanced is False:
                logging.info('go_to_next_service: No action taken. Is there a service after this one?')
