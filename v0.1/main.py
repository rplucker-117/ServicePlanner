...
# May God help your soul if you try to edit this code
...
def create_ui(plan_items):
    ...
    for iteration, item in enumerate(plan_items):
        line_height=40
        # count number of lines in app_cues_verbose, use it as item height * 18.
    ...
    def next(cue_items):
        if cue_items:
            # Finds next item that's not a header, for use in cueing
    ...
    btn_previous = Button(service_controls_frame,
                          ...
                          command=previous).grid(row=0, column=0)
    ...
    btn_next = Button(service_controls_frame,
                      ...
                      fg=accent_text_color,
                      font=(accent_text_font, accent_text_size),
                      command=lambda: next(cue_items=True)).grid(row=0, column=1)
