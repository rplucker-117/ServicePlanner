...
service_types_data = pco_plan_update.get_service_types()[0]

logging.debug('Creating ui...')
# create list of service folder type names
service_types = []
for service_type in range(len(service_types_data['data'])):
    service_types.append(service_types_data['data'][service_type]['attributes']['name'])
...
def select_service_folder_from_list(value):
    ...
    service_type_id = service_types_data['data'][service_type_index]['id']
...
def create_ui(plan_items):
    ...
    def load_adjacent_plan():
        ...
        def create_adjacent_plan_view(service_type_id, service_id):
            ...
            service_date = pco_plan_update.get_service_details_from_id(service_type_id=service_type_id, service_id=service_id)[1]['date']

            service_item_data = pco_plan_update.get_service_items(service_type_id=service_type_id,
                                                             service_id=service_id)[1]

            # service topography
            service_topo_text = service_type_name + '  >  ' + service_date
            ...
            def refresh():
                ...
                live_item = pco_live.get_current_live_item(service_type=service_type_id,
                                                           plan=service_id)
                live_item_title = pco_plan_update.get_item_details_from_item_id(service_type=service_type_id,
                                                                                plan=service_id, item_id=live_item)[0]
                next_item = pco_live.find_next_live_item(service_type_id=service_type_id, service_id=service_id)

                current_item_label.configure(text=live_item_title)
                next_item_label.configure(text=next_item['title'])
                if next_item['type'] == 'song':
                    next_item_label.configure(fg=song_title_color)

                adjacent_plan_frame.after(adjacent_plan_refresh_interval, refresh)
